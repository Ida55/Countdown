...
function timerOn() {
    ...
    // Ou sinon à chaque fois qu'une seconde touche à sa fin -> (enclenche la) diminution de la seconde suivante et après 59 secondes les minutes diminuent à leur tours
      tmScs--;
      if(tmScs < 0) {
        if(tmMin > 0) {
          tmScs = 59;
    ...
    /* les 2 premières lignes sont celles qui permettent la visibilité du compte à rebours en action sur la page html
    le setTimeout permet de faire fonctionner la fonction timerOn du dessus (et donc le mécanisme de décompte) 
    toute les secondes (sinon sans ça il resterait bloqué à 00:00) */
  ...
  function timerPause() {
    tmScs = 0;
    tmMin = 0;
    /* (ce que le code devra exécuter ->) si l'utilisateur décide de stopper le compteur à tout moment en cliquant sur le bouton 'OFF' le compteur revient à 00:00 */
  }
